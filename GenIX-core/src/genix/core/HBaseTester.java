...
import data.io.GenixFileProvider;
...
public class HBaseTester {
    ...
    try {
      HBaseGeneAdapter.initTable( config , tabName );
    }
    catch(Exception ex){
      System.out.println(">>> Table ["+tabName+"] exists ... ;-)");
    }
    ...
    String[] dat = GenixFileProvider.getSample();
    ...
    p.add(Bytes.toBytes("gfr"), Bytes.toBytes("raw"),
      Bytes.toBytes( value ));
    ...
    Get g = new Get(Bytes.toBytes("gfr"));
    Result r = table.get(g);
    byte [] value2 = r.getValue(Bytes.toBytes("raw"), Bytes.toBytes(key));
    // If we convert the value bytes, we should get back 'Some Value', the
    // value we inserted at this location.
    String valueStr = Bytes.toString(value2);
    ...
    static String getRandomKeyName() {
